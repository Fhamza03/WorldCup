...
import { updateAccommodation } from "@/api/accomondation/accomondation.api";

interface UpdateAccommodationProps {
  onClose?: () => void;
...
export default function UpdateAccommodation({
  ...
  const [user] = useState<User>({
    userId: AccommodationData?.provider?.userId || 1,
    email: AccommodationData?.provider?.email || "",
    password: "",
    firstName: AccommodationData?.provider?.firstName || "",
    lastName: AccommodationData?.provider?.lastName || "",
    birthDate: AccommodationData?.provider?.birthDate || "",
    nationality: AccommodationData?.provider?.nationality || "",
    nationalCode: AccommodationData?.provider?.nationalCode || "",
    profilePicture: AccommodationData?.provider?.profilePicture || "",
  });
  ...
  // Ensure form data is updated if AccommodationData changes
  useEffect(() => {
    if (AccommodationData) {
      setFormData(AccommodationData);
    }
  }, [AccommodationData]);
  ...
  function validateAccommodation(formData: Accommodation): {
    ...
    // For updates, we need to validate the ID
    if (!formData.idAccommodation || formData.idAccommodation <= 0)
      errors.push("Invalid accommodation ID.");

    if (!formData.address || formData.address.trim() === "")
      errors.push("Address is required.");
    if (formData.roomsCount <= 0)
      errors.push("Rooms count must be greater than 0.");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await updateAccommodation(formData);
      ...
      console.log("Updating accommodation:", formData);
