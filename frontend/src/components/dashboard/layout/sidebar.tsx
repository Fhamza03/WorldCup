...
const Sidebar = () => {
  ...
  useEffect(() => {
    const activeItem = sidebarItems.find((item) => pathname === item.to);
    if (activeItem) {
  ...
  return (
    <aside
      className={`bg-white shadow p-4 flex flex-col h-screen transition-all duration-300 ease-in-out ${
        isExpanded ? "w-64" : "w-20"
      }`}
    >
      <div
        ...
        {isExpanded ? (
          <>
            <div className="flex items-center overflow-hidden">
              ...
              <h1 className="text-xl font-bold whitespace-nowrap text-black">
                Transport Hub
              </h1>
            </div>
            <button
      ...
      <nav className="flex flex-col">
        {sidebarItems.map((item) => {
          const isActive = pathname === item.to || activeId === item.id;
          return (
            <button
              key={item.id}
              onClick={() => {
      ...
      <div className="mt-auto pt-6 flex-grow flex flex-col justify-end">
        ...
        >
          <Home
            className={isExpanded ? "mr-3 ml-2" : "mx-auto"}
            color="#27ae60"
            size={24}
            strokeWidth={1.5}
          />
